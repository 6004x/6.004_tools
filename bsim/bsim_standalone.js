...
$(function() {
    ...
    function setup_tab(name,contents) {
        // if name starts with "*" mark buffer as read-only
        var read_only = false;
        if (name[0] == '*') {
            read_only = true;
            name = name.substr(1);
        ...
        var doc = editor.openTab(name,load ? 'Loading '+contents : contents,true,null,read_only);
        if (load) {
    ...
    if (initial_data) $.extend(configuration,JSON.parse(initial_data));

    // augment with any state saved on the server
    $.ajax(window.location.origin,{
        ...
        success: function(result) {
            // update configuration with saved state
            if (result) $.extend(configuration,JSON.parse(result));

            // set up the buffers
            editor.closeAllTabs();
            if (configuration.initial_state)
                $.each(configuration.initial_state,setup_tab);
            if (configuration.state)
                $.each(configuration.state,setup_tab);
    ...
    editor.save_to_server = function (callback) {
        ...
        $.ajax(window.location.origin,{
            type: 'POST',
            dataType: 'text',
            data: {key: window.location.pathname, value: JSON.stringify(state)},
            error: function(jqXHR, textStatus, errorThrown) {
