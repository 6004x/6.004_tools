var plot = (function() {
    ...
    function dataset_plot(dataset) {
        ...
        for (t = dataset.xtick[0]; t < dataset.xtick[2]; t += dataset.xtick[1]) {
            ...
            c.stroke();
            c.fillText(jade_utils.engineering_notation(t, 2)+xunits, temp, dataset.top + dataset.hplot);
        }
        ...
        if (dataset.yunits) {
            ...
            for (t = dataset.ytick[0]; t < dataset.ymax; t += dataset.ytick[1]) {
                ...
                c.stroke();
                c.fillText(jade_utils.engineering_notation(t, 2)+dataset.yunits,dataset.left-2,temp);
            }
            c.stroke();
        }
    ...
    function graph_redraw(dataseries) {
        ...
        $.each(dataseries,function(index,dataset) {
            ...
            if (dataseries.cursor !== undefined) {
                ...
                // add x-axis label
                var label = jade_utils.engineering_notation(x,1);
                ...
                // now add label
                if (dataset.yunits) {
                    label = '='+jade_utils.engineering_notation(y,1) + dataset.yunits;
                    c.font = graph_legend_font;
