var assembleSpringyRaphael = function(container, parsedDict) {

	// connection function taken from: https://github.com/DmitryBaranovskiy/raphaeljs.com/blob/master/graffle.js
	Raphael.fn.connection = function (obj1, obj2, style) {
	    ...
	    var edge = {/*
	        ...
	        draw : function() {
	            ...
	            /* bezier path (makes the path have a curve) */
	            var x1 = p[res[0]].x,
	                y1 = p[res[0]].y,
	                x4 = p[res[1]].x,
	                y4 = p[res[1]].y,
	...
	// object relating statename to node
    var states_to_node = {};
	...
	for (key in parsedDict.states[0].args) {
    	if (typeof parsedDict.states[0].args[key] === 'string') { // the array contains some functions too
	    	...
	    	states_to_node[state] = graph.newNode({label: state});
	    }
    }

    console.log("states_to_node: ");
    console.log(states_to_node);
	...
	for (key in parsedDict.action) {
    	if (parsedDict.action.hasOwnProperty(key)) {
	    	...
	    	if (action[2] === "*halt*") {
	    		...
	    		graph.newEdge(states_to_node[action[0]], states_to_node[action[0]], {color: '#00A0B0'});
	    	}
	    	else {
	    		graph.newEdge(states_to_node[action[0]], states_to_node[action[2]], {color: '#00A0B0'});
	    	}
    	}
    }
	...
	function doit() {
	    ...
	    var r = Raphael("holder_springy_raphael", 640, 480);
	        ...
	        function drawEdge(edge, p1, p2) {
	            ...
	            if (!edge.connection) {
	                if (!edge.source.shape || !edge.target.shape)
	                    return;
	                // create the connection for the first time
	                connection = r.connection(edge.source.shape, edge.target.shape, {stroke: edge.data['color'], directed: true});
	                edge.connection = connection;
	            } else {
	            	// re-draw the connection as the shapes move into place
